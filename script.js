document.addEventListener('DOMContentLoaded', () => {
    ...
    // Custom Word Dictionary (BARU: input terpisah)
    ...
    const addPairedWordButton = document.getElementById('addPairedWordButton'); // ID Tombol diubah
    ...
    let players = [];
    let civilianWord = '';
    let undercoverWord = ''; // Bisa jadi kata pasangan atau kata acak
    let defaultWords = [
    ...
    let customWords = []; // Sekarang akan menyimpan objek: { civilian: "kata1", undercover: "kata2" }
    ...
    const truths = ["Apa kebohongan terbesarmu?", /* ... Tambahkan lebih banyak ... */];
    const dares = ["Nyanyikan lagu anak-anak dengan gaya rock.", /* ... Tambahkan lebih banyak ... */];
    // (Pastikan daftar truth dan dare Anda sudah diisi)
    truths.push(
    ...
    function updateActiveSection(activeSectionId) {
        ...
        // Section kamus kata dikontrol terpisah (lihat setupGame dan resetGame)
    }

    function loadCustomWords() {
        try {
            const stored = localStorage.getItem('customPairedWords_HTV4'); // Key baru untuk struktur baru
            if (stored) {
    ...
    function updateCustomWordListUI() {
        ...
        customWords.forEach((pair, index) => {
            ...
            li.textContent = `${pair.civilian} / ${pair.undercover}`; // Tampilkan pasangan
    ...
    function loadSavedPlayerNames() { /* ... (sama seperti sebelumnya, gunakan key _HTV4) ... */ }
    function savePlayerNames(namesArray) { /* ... (sama seperti sebelumnya, gunakan key _HTV4) ... */ }
    function generatePlayerNameInputs() { /* ... (sama seperti sebelumnya) ... */ }
    ...
    // BARU: Listener untuk tombol tambah pasangan kata
    addPairedWordButton.addEventListener('click', () => {
        ...
        if (civWord && undWord) {
            ...
            // Cek duplikasi pasangan (dalam urutan apa pun atau salah satu kata sudah ada di pasangan lain)
    ...
    revealRoleCardButton.addEventListener('click', () => { /* ... (sama seperti sebelumnya) ... */ });
    flipCardButton.addEventListener('click', () => { /* ... (sama seperti sebelumnya) ... */ });
    closePopupButton.addEventListener('click', () => { /* ... (sama seperti sebelumnya) ... */ });
    cardPopupOverlay.addEventListener('click', (e) => { if (e.target === cardPopupOverlay) closePopupButton.click(); });
    checkMyWordAgainButton.addEventListener('click', () => { /* ... (sama seperti sebelumnya) ... */ });
    activateVotingButton.addEventListener('click', () => { /* ... (sama seperti sebelumnya) ... */ });
    playersDisplay.addEventListener('click', (event) => { /* ... (sama seperti sebelumnya) ... */ });
    confirmVoteYesButton.addEventListener('click', () => { /* ... (sama seperti sebelumnya) ... */ });
    confirmVoteNoButton.addEventListener('click', () => { /* ... (sama seperti sebelumnya) ... */ });
    submitMrWhiteGuessButton.addEventListener('click', () => { /* ... (sama seperti sebelumnya) ... */ });
    getTruthButton.addEventListener('click', () => { /* ... (sama seperti sebelumnya) ... */ });
    getDareButton.addEventListener('click', () => { /* ... (sama seperti sebelumnya) ... */ });
    punishmentDoneButton.addEventListener('click', () => { /* ... (sama seperti sebelumnya) ... */ });
    restartGameButton.addEventListener('click', () => { /* ... (sama seperti sebelumnya) ... */ });

    revealRoleCardButton.addEventListener('click', () => { playSound(clickSound); if (currentPlayerIndexForWordReveal >= players.length) return; const p = players[currentPlayerIndexForWordReveal]; let txt = ""; if (p.role === "Mr. White") txt = `Kamu <strong>Mr. White</strong>.<br><br>Berpura-pura tahu & jangan ketahuan!`; else if (p.role === "Undercover") txt = `Kamu <strong>Undercover</strong>.<br><br>Kata beda: <strong class='highlight-word'>${p.word}</strong>.<br><br>Cari Civilian!`; else txt = `Kamu <strong>Civilian</strong>.<br><br>Kata: <strong class='highlight-word'>${p.word}</strong>.<br><br>Cari Undercover & Mr. White!`; popupCardWord.innerHTML = txt; cardPopupOverlay.classList.remove('hidden'); wordCardFlipper.classList.remove('is-flipped'); currentWordRevealCallback = proceedToNextPlayerOrVoting; });
    ...
    checkMyWordAgainButton.addEventListener('click', () => { playSound(clickSound); const name = prompt("Nama Anda:"); if (name) { const p = players.find(pl => pl.name.toLowerCase() === name.toLowerCase().trim() && !pl.isOut); if (p) { let txt = ""; if (p.role === "Mr. White") txt = `Hai ${p.name}, kamu <strong>Mr. White</strong>. Tidak ada kata.`; else txt = `Hai ${p.name}, kata: <strong class='highlight-word'>${p.word}</strong>`; popupCardWord.innerHTML = txt; cardPopupOverlay.classList.remove('hidden'); wordCardFlipper.classList.remove('is-flipped'); currentWordRevealCallback = null; } else { alert("Pemain tidak ditemukan/keluar."); } } });
    ...
    function setupGame() {
        ...
        // --- PERUBAHAN LOGIKA PEMILIHAN KATA ---
        const wordOpportunities = [
            ...defaultWords.map(w => ({ type: 'single', word: w })),
            ...customWords.map(p => ({ type: 'pair', pair: p })) // customWords berisi {civilian, undercover}
        ];

        if (wordOpportunities.length === 0) {
            roleCountError.textContent = "Kamus kata (default dan pribadi) kosong!"; return;
        }
        // Jika ada Undercover dan total opportunity kata kurang dari 2 (kecuali hanya ada 1 opportunity total)
        // Ini menjadi lebih kompleks karena opportunity bisa berupa pair.
        // Cukup pastikan ada sesuatu untuk dipilih. Jika hanya ada 1 pair, itu cukup. Jika hanya 1 single, itu cukup.
        // Validasi yang lebih penting adalah jika Undercover butuh kata berbeda.
        ...
        if (chosenOpportunity.type === 'pair') {
            ...
            undercoverWord = chosenOpportunity.pair.undercover; // Langsung dari pasangan
        } else { // type === 'single'
            civilianWord = chosenOpportunity.word;
            // Untuk Undercover, pilih kata lain yang berbeda dari civilianWord
            // dari semua kemungkinan kata tunggal (default + unpacked custom pairs)
            let flatWordList = [
                ...defaultWords,
                ...customWords.flatMap(p => [p.civilian, p.undercover])
            ].filter((value, index, self) => self.indexOf(value) === index); // Buat unik

            if (flatWordList.length <= 1 || (flatWordList.length === 1 && flatWordList[0] === civilianWord)) {
                undercoverWord = civilianWord; // Terpaksa sama jika hanya ada 1 kata
            } else {
        ...
        // Jika tidak ada undercover, undercoverWord tidak terlalu penting
        if (numUnds === 0) {
            undercoverWord = civilianWord; // Atau bisa null, tapi ini konsisten
        }
        ...
        names.forEach((name, i) => {
            ...
            if (role === "Undercover") wordForPlayer = undercoverWord; // Undercover dapat kata sesuai logika di atas
            // Mr. White tetap null
    ...
    function proceedToNextPlayerOrVoting() { /* ... (sama seperti sebelumnya) ... */ }
    function updatePlayersDisplayUI() { /* ... (sama seperti sebelumnya, pastikan data-player-id ditambahkan) ... */ }
    function deactivateVotingMode() { /* ... (sama seperti sebelumnya) ... */ }
    function triggerTruthOrDare(player, reason) { /* ... (sama seperti sebelumnya) ... */ }
    function checkGameContinuationOrEnd() { /* ... (sama seperti sebelumnya) ... */ }
    function endGame(message) { /* ... (sama seperti sebelumnya) ... */ }
    function savePlayerScores() { /* ... (sama seperti sebelumnya, gunakan key _HTV4) ... */ }
    function displayScoresUI() { /* ... (sama seperti sebelumnya) ... */ }
    function resetGame() { /* ... (sama seperti sebelumnya) ... */ }
