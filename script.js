document.addEventListener('DOMContentLoaded', () => {
    ...
    function resetGameFull() {
        ...
        totalPlayersConfigInput.value = "3";
        civiliansConfigInput.value = "1"; // Default Civilian tetap 1
    ...
    function validateConfigAndProceed() {
        ...
        // Validasi civ < 1 dihapus, karena Civilian boleh 0
        if (uc < 0 || mw < 0) { showConfigError("Jumlah Undercover atau Mr. White tidak boleh negatif."); return; } // Sebenarnya sudah dihandle min="0" di HTML

        // BARU: Validasi Undercover + Mr. White minimal 1
        if (uc === 0 && mw === 0) {
            showConfigError("Permainan harus memiliki setidaknya 1 Undercover atau 1 Mr. White.");
            return;
        }
    ...
    function checkWinConditionsAndProceed() {
        ...
        // NEW CONDITION: 1 Civilian vs 1 Undercover duel (and no Mr. White)
        if (activePlayers.length === 2 && activeCivilians.length === 1 && activeUndercovers.length === 1 && activeMrWhite.length === 0) {
            gameShouldEnd = true;
            winType = "UC_DUEL_WIN";
            winners = activeUndercovers; // The lone Undercover wins
        }
        // Existing condition: All Civilians eliminated
        ...
        // Existing condition: All Undercovers and Mr. Whites eliminated
    ...
    function announceWinner(winningTeamType, winnerPlayerObjects = []) {
        ...
        switch (winningTeamType) {
            ...
            case "UC_DUEL_WIN": // New case for 1v1 Undercover win
        ...
        popupWinnerMessageH2.textContent = popupMsg;
        popupWinnerDetailP.textContent = popupDetail;
        ...
        players.forEach(player => {
            ...
            switch (currentWinningTeamType) {
                ...
                case "CIVILIAN_TEAM":
                    ...
                    break;
                case "UC_DUEL_WIN": // New case for 1v1 Undercover win score
                    if (player.originalRole === "Undercover" && isWinnerThisRoundObject) {
                        roundScore = POINTS_UC_WIN;
                    }
                    break;
            ...
            player.roundScoreThisTurn = roundScore;
        });
    }
