document.addEventListener('DOMContentLoaded', () => {
    ...
    // ... (semua referensi DOM lainnya tetap sama) ...
    ...
    // Menu Elements
    alert("Mencoba mengambil elemen menu..."); // DIAGNOSTIK ALERT 1
    ...
    if (!menuButton) alert("ERROR: menuButton tidak ditemukan!"); // DIAGNOSTIK ALERT
    if (!menuPanel) alert("ERROR: menuPanel tidak ditemukan!");   // DIAGNOSTIK ALERT
    if (!menuOverlay) alert("ERROR: menuOverlay tidak ditemukan!"); // DIAGNOSTIK ALERT


    // Game State
    // ... (semua game state tetap sama) ...
    let players = [];
    let civilianWord = '';
    let undercoverWord = '';
    let defaultWords = [
    ...
    function toggleMenu(forceClose = false) {
        alert("Fungsi toggleMenu dipanggil. forceClose: " + forceClose); // DIAGNOSTIK ALERT 2
        if (!menuPanel || !menuOverlay) {
            alert("ERROR: menuPanel atau menuOverlay null di dalam toggleMenu!");
            return;
        }

        if (forceClose || menuPanel.classList.contains('open')) {
            alert("Menutup menu..."); // DIAGNOSTIK ALERT 3A
            menuPanel.classList.remove('open');
            menuOverlay.classList.add('hidden');
        } else {
            alert("Membuka menu..."); // DIAGNOSTIK ALERT 3B
            menuPanel.classList.remove('hidden'); // Hapus 'hidden' agar display tidak none
            menuPanel.classList.add('open');    // Tambah 'open' untuk animasi slide
            menuOverlay.classList.remove('hidden'); // Tampilkan overlay
        }
        alert("Kelas menuPanel sekarang: " + menuPanel.className); // DIAGNOSTIK ALERT 4
        alert("Kelas menuOverlay sekarang: " + menuOverlay.className); // DIAGNOSTIK ALERT 5
    }

    // --- Menu Event Listeners ---
    if (menuButton) {
        menuButton.addEventListener('click', () => {
            alert("Tombol Menu DIKLIK!"); // DIAGNOSTIK ALERT 6
            playSound(clickSound);
            toggleMenu();
        });
    } else {
        alert("ERROR: Event listener untuk menuButton tidak bisa ditambahkan karena elemen tidak ditemukan.");
    }
    ...
    if (menuKamusKata) {
        menuKamusKata.addEventListener('click', () => {
            ...
            if (customWordSection) customWordSection.classList.toggle('hidden');
            if (gameSetupSection && customWordSection) gameSetupSection.classList.toggle('hidden', !customWordSection.classList.contains('hidden'));
            ...
            if (customWordSection && !customWordSection.classList.contains('hidden')) {
                if(gameSetupSection) gameSetupSection.classList.add('hidden');
                 ...
                 if (!gameInProgress && gameSetupSection) {
        ...
        menuModeParty.addEventListener('click', () => {
            ...
            if (numFoolsInput) {
                ...
                if (foolInputDiv) {
                    ...
                    setTimeout(() => { foolInputDiv.style.backgroundColor = ''; }, 600);
                }
            }
            toggleMenu(true);
        });
    ...
    // ... (sisa fungsi loadCustomWords, saveCustomWords, updateCustomWordListUI, loadSavedPlayerNames, savePlayerNames, generatePlayerNameInputs tetap sama) ...
    loadCustomWords = () => {
        try {
    ...
    // ... (sisa event listener dan fungsi game logic seperti toggleMusicButton, addPairedWordButton, startGameButton, revealRoleCardButton, dll tetap sama persis seperti versi lengkap terakhir) ...
    // Pastikan semua fungsi ini ada dan tidak terhapus. Saya hanya fokus pada bagian menu di atas.
    // Untuk keringkasan, saya tidak menyalin ulang semua fungsi game logic lainnya di sini,
    // tapi Anda harus memastikan mereka tetap ada di file script.js Anda dari versi lengkap terakhir.
    // Saya akan salin ulang bagian penting yang mungkin terpengaruh:
    ...
    function initializeApp() {
        ...
        alert("Aplikasi dimuat! Coba klik tombol menu (garis tiga)."); // DIAGNOSTIK ALERT TERAKHIR
    }
