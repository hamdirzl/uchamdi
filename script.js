document.addEventListener('DOMContentLoaded', () => {
    ...
    if (toggleMusicBtn && backgroundMusic) {
        toggleMusicBtn.addEventListener('click', () => {
            ...
            if (isMusicPlaying) {
                ...
                musicManuallyPaused = false;
                backgroundMusic.play().then(() => { isMusicPlaying = true; toggleMusicBtn.innerHTML = '🔇'; }).catch(() => { isMusicPlaying = false; musicManuallyPaused = true; toggleMusicBtn.innerHTML = '⚠️'; });
            }
        });
    }
    ...
    function validateConfigAndProceed() {
        ...
        // Jika input kosong, anggap nilainya "0". Jika ada isinya, gunakan isi tersebut.
        const uc = parseInt(undercoversConfigInput.value || "0");
        const mw = parseInt(mrWhitesConfigInput.value || "0");
    
        configErrorP.classList.add('hidden'); configErrorP.textContent = '';
        
        // Pengecekan isNaN akan tetap berfungsi jika pengguna mengetik teks (misal: "abc")
        if (isNaN(total) || isNaN(civ) || isNaN(uc) || isNaN(mw)) { 
            showConfigError("Semua field jumlah harus diisi dengan angka."); 
            return; 
        }
        if (total < 3) { 
            showConfigError("Jumlah total pemain minimal 3."); 
            return; 
        }
        // Pengecekan nilai negatif tetap diperlukan
        if (civ < 0 || uc < 0 || mw < 0) { 
            showConfigError("Jumlah peran tidak boleh negatif."); 
            return; 
        }
        ...
        if (civ + uc + mw !== total) { 
            showConfigError(`Jumlah peran (${civ + uc + mw}) tidak sama dengan Total Pemain (${total}).`); 
            return; 
        }
    
        configuredTotalPlayers = total;
        initialCiviliansCount = civ; 
        initialUndercoversCount = uc; 
        initialMrWhitesCount = mw;
    ...
    function checkWinConditionsAndProceed() {
        ...
        if (activePlayers.length <= 0) {
             winDetected = true; determinedWinType = "NO_WINNER_DRAW";
        } else if (activePlayers.length === 2 && activeCivilians.length === 1 && activeUndercovers.length === 1 && activeMrWhite.length === 0) {
            winDetected = true; determinedWinType = "UC_DUEL_WIN";
            ...
            else { determinedWinType = "NON_CIVILIAN_SURVIVAL"; determinedWinners = activePlayers; }
        ...
        if (winDetected) {
            ...
            if (determinedWinType === "UC_DUEL_WIN") {
